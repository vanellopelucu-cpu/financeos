...
export function EditPocketModal({ open, onClose, pocket, onSave }: EditPocketModalProps) {
...
function EditPocketForm({ pocket, currency, onSave, onCancel }: EditPocketFormProps) {
  const [name, setName] = useState(pocket.name)
  const [icon, setIcon] = useState(pocket.icon)
  const [currentAmount, setCurrentAmount] = useState(String(pocket.currentAmount))
  const [targetAmount, setTargetAmount] = useState(String(pocket.targetAmount))

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    await onSave(pocket.id, {
      name: name.trim(),
      icon,
